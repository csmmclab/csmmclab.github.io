...
effectname = {'A','B','C','E','F','G','H','Ml','Mq','Mc','AB','AC','AD','AE','AF','AG','AH','AL','AMl','AMq','AMc','BC','BD','BE','BF','BG','BH','BL','BMl','BMq','BMc','CD','CE','CF','CG','CH','CL','CMl','CMq','CMc','DE','DF','DG','DH','DL','DMl','DMq','DMc','EF','EG','EH','EL','EMl','EMq','EMc','FG','FH','FL','FMl','FMq','FMc','GH','GL','GMl','GMq','GMc','HMl','HMq','HMc','LMl','LMq','LMc','MlMq','MlMc','ABL','ABMl','ABMq','ABMc','ACL','ACMl','ACMq','ACMc','ADL','ADMl','ADMq','ADMc','AEL','AEMl','AEMq','AEMc','AFL','AFMl','AFMq','AFMc','AGL','AGMl','AGMq','AGMc','AHL','AHMl','AHMq','AHMc','ALMl','ALMq','ALMc','BLMl','BLMq','BLMc','CLMl','CLMq','CLMc','DLMl','DLMq','DLMc','ELMl','ELMq','ELMc','FLMl','FLMq','FLMc','GLMl','GLMq','GLMc','HLMl','HLMq','HLMc'}';

fprintf('%15s','Effect','Y-mean');
fprintf('\n');
for i = 1:126
    fprintf('%15s',char(effectname(i)));
    fprintf('%14.3f',ymbetam(i));
    fprintf('\n');
end
...
text(phiinv(123)-0.1,theta(123)-0.004,effectname(idx(123)));
for i = 124:126
    text(phiinv(i)-0.1,theta(i)+0.005,effectname(idx(i)));
end
xlabel('\phi^{-1}(p(i))');
ylabel('absolute effects');
title('Half-normal plot of location effects');
axis([-0.02 3 -0.01 0.2]);
