...
%% Normal plot of location effects
[theta,idx] = sort(ymbeta);
p = ((1:length(ymbeta))-0.5)/length(ymbeta);
phiinv = norminv(p,0,1);
figure('color','w')
plot(phiinv,theta,'.','MarkerSize',10);
for i = 1:length(ymbeta)
    text(phiinv(i),theta(i)-0.03,effectname(idx(i)));
end
xlabel('\phi^{-1}(p(i))');
ylabel('effects');
title('Normal plot of location effects');
